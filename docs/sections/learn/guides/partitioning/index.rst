...
You can find all of the code for this guide checked into the dagster repository at

``dagster/examples/dagster_examples/stocks``

When working with large amounts of data, the data usually needs to be partitioned into smaller units that can be processed more quickly and easily.

In this guide, we'll learn about why we partition data and how to write Dagster pipelines that can easily work with partitioned data. We'll also explore how to schedule partioned work using Dagster's built-in scheduler functionalities.
...
For example, lets say we have a metrics pipeline that partions data based on 24 hour time intervals. We will probably want to schedule this pipeline to run every day at midnight on the previous 24 hours' metrics data. With this setup, we'll have data continuously processed for every 24 hour segment.

Traditional workflow systems tightly couple partitioning and scheduling and provide solutions to date partitioned schedules like this metrics pipeline. However, as we've seen, there are many other ways to partion data, and these two concepts do not need to be completely intertwined. Being able to partition data on axis other than time is especially useful when conducting backfills.

Dagster aims to provides a flexible and generalized solution to partitioning, backfills, and scheduling that can extend beyond datetime partitioning, while also providing a set of easy to use tools for the common datetime partioning use case.

In the next part of the tutorial, we'll build a partioned schedule.
